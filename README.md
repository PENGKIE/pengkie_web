...
```bash
web$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
web$ sam local start-api
...
```bash
web$ sam logs -n HelloWorldFunction --stack-name web --tail
```
